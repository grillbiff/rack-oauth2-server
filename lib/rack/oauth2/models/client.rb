    ...
    class Server
      ...
      class Client
        ...
        class << self
          ...
          def lookup(field)
            id = BSON::ObjectId.from_string(field.to_s)
            Server.new_instance self, collection.find({:_id => id}).limit(1).one
          rescue BSON::ObjectId::Invalid
            Server.new_instance self, collection.find({ :display_name=>field }).limit(1).one || collection.find({ :link=>field }).limit(1).one
          end
        ...
        Server.create_indexes do
          ...
          # client from a URL.
          collection.indexes.create({display_name: 1},{ascending: 1})
          collection.indexex.create({link: 1},{ascending: 1})
        end
      end
