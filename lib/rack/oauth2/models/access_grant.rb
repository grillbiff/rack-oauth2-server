    ...
    class Server
      ...
      class AccessGrant
        class << self
          ...
          def from_code(code)
            Server.new_instance self, collection.find({ :_id=>code, :revoked=>nil }).limit(1).one
          end
...
#          reload = self.class.collection.find_one({ :_id=>code, :revoked=>nil }, { :fields=>%w{access_token} })
          reload = self.class.collection.find({ :_id=>code, :revoked=>nil }).select(access_token: 1, _id: 0).limit(1).one
